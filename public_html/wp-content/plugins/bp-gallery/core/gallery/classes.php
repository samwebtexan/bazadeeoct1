...
/** single will be used in the next release*/ //v1.1 inspiration from BP_Groups_Group class at some extent
	function __construct( $id = null, $single = false, $get_user_dataset = false ) {
		if ( $id ) {
			$this->id = $id;
...
function delete_gallery_for_user($user_id){
       self::delete_gallery_for_owner("user",$user_id);
  }
 function delete_gallery_for_owner($owner_type,$owner_id){
    global $bp,$wpdb;
    $query="SELECT id from {$bp->gallery->table_galleries_data} WHERE owner_object_type=%s AND owner_object_id=%d";
    $galleries=$wpdb->get_col($wpdb->prepare($query,$owner_type,$owner_id));
...
function delete_gallery_for_group($group_id){
       self::delete_gallery_for_owner("groups",$group_id);
   }
/*
 *  Helper Functions
 */
...
function get_order($owner_type,$owner_id,$gallery_type){
    global $bp,$wpdb;
        ...
        if ( $owner_type)
            $where_conditions[] = $wpdb->prepare( "g.owner_object_type =%s",$owner_type);

        if ( $owner_id)
            $where_conditions[] = $wpdb->prepare( "g.owner_object_id =%d",$owner_id);

        if($gallery_type)
            $where_conditions[] = $wpdb->prepare( "g.gallery_type =%s",$gallery_type);
		// Build where sql statement if necessary

        if ( !empty( $where_conditions ) )
            $where_sql = 'WHERE ' . join( ' AND ', $where_conditions );

        $sql="SELECT max(sort_order) as sort_order from {$bp->gallery->table_galleries_data} g {$where_sql}";
...
case "date":
    $sql="ORDER BY g.date_updated {$sort_order}";
    break;
case "alphabet":
    $sql="ORDER BY g.title {$sort_order} ";
    break;
case "sort_order":
default:
     $sql="ORDER BY g.sort_order {$sort_order}";
 break;
...
function get_all($status_filter=array('public'),$gallery_type=null,$owner_type=null,$owner_id=null,$limit = null, $page = null,$search_terms=null, $order_by = "sort_order",$sort_order="ASC" ) {
    ...
    foreach($status_filter as $key=>$ac_level)
      $access_cond[]=$wpdb->prepare("g.status=%s",$ac_level);
    ...
    $order_sql = self::get_orderby_sql($order_by,$sort_order) ;//"ORDER BY g.sort_order $order";
...
if(!empty($gids)){
    ...
    $where_conditions[]="g.owner_object_type='groups' AND g.owner_object_id IN {$gids} ";
     // Build where sql statement if necessary
    if ( !empty( $where_conditions ) )
	  $where_sql = 'WHERE ' . join( ' AND ', $where_conditions );
...
function total_gallery_count($owner_type=null,$owner_id=null, $access=array("public")){
  ...
  if ( !empty( $where_conditions ) )
    ...
    //now the sql
    $q= "SELECT COUNT(DISTINCT id) FROM {$bp->gallery->table_galleries_data} g  {$where_sql} ";
...
function get_gallery_count_by_type($gallery_type, $owner_type, $owner_id, $status){
    ...
    if ( $status )
        $where_conditions[] = $wpdb->prepare( "g.status = %s",$status );
...
function get_id_from_slug( $slug,$owner_type,$owner_id ) {
	return self::gallery_exists( $slug,$owner_type,$owner_id );
}
