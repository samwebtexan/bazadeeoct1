...
function bp_gallery_page_title($complete_title,$title, $sep, $seplocation){
   global $bp;
   $title_g ='';
   if(bp_is_current_component($bp->gallery->slug)||$bp->current_action==$bp->gallery->slug) { // echo bp_is_single_gallery();
     if(bp_is_single_gallery())
            $title_g = $bp->gallery->current_gallery->title;
     if(bp_is_single_media())
             $title_g = $title_g. ' '. $sep. ' '.$bp->gallery->current_media->title;
     if($title_g)
        return $complete_title.' '.$title_g .' '. $sep. ' ';
        }
return $complete_title;
}
add_filter("bp_modify_page_title","bp_gallery_page_title",20,4);
...
function gallery_filter_activity_status($show_hidden,$gallery){
 ...
 if($gallery->owner_object_type=="user")
     return $show_hidden;
 if($gallery->owner_object_type="groups"&&  class_exists('BP_Groups_Group')){
