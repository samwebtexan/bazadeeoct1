...
function delete(){
   ...
   //remove from activity stream too
    ...
    $sql="DELETE FROM {$bp->gallery->table_media_data} WHERE id=%d";
       //now delete from the database
     if($wpdb->query($wpdb->prepare($sql,$this->id))){
...
function get_all( $status_filter=array('public'),$type=null,$gallery_id=null,$user_id=null,$limit = null, $page = null, $order_by = "sort_order",$sort_order=null ) {
    ...
    if ( $gallery_id )
    	$where_conditions[] = $wpdb->prepare( "m.gallery_id =%d",$gallery_id);//limit to gallery
     if($type)
         $where_conditions[] = $wpdb->prepare( "m.type =%s",$type);//limit to gallery
         //
$where_conditions[] = "m.gallery_id =g.id";//limit to gallery
//
if($user_id)
    $where_conditions[] = $wpdb->prepare( "m.user_id =%d",$user_id);//limit to gallery
...
// Build where sql statement if necessary
    ...
    $sql =  "SELECT m.id FROM {$bp->gallery->table_media_data} m,{$bp->gallery->table_galleries_data} g {$where_sql} {$order_sql} {$pag_sql}" ;

    $media=$wpdb->get_col($sql);
    $total_media = $wpdb->get_var(  "SELECT COUNT(m.id)From {$bp->gallery->table_media_data} m,{$bp->gallery->table_galleries_data} g {$where_sql}"  );
...
function get_adjacent_gallery_media($previous = true,$status_filter=null,$media=null) {
...
case "random":
    ...
    case "date":
          $sql="ORDER BY m.date_updated $order";
    ...
    case "sort_order":
    default:
         $sql="ORDER BY m.sort_order $order";
...
//get the order of current gallery based on the owner object type,owner object id and the gallry type
    global $bp,$wpdb;
    $where_sql = null;
    $where_sql = $wpdb->prepare( " WHERE m.gallery_id =%d",$gallery_id);
    $sql="SELECT max(sort_order) as sort_order from {$bp->gallery->table_media_data} m {$where_sql}";
...
//force access level
 ...
 if($gallery_id)
   $where_conditions[]=$wpdb->prepare("gallery_id = %d",$gallery_id);
