...
function gallery_action_create_gallery( $owner_type = 'user', $owner_id = null ) {
	...
	if ( empty( $_POST['gallery_title'] )  )  {
		bp_core_add_message( __( 'Please fill the title.', 'bp-gallery' ), 'error' );
             ...
             if( ( $id = gallery_create_gallery( array(
                                          'title'               => stripslashes($_POST["gallery_title"]),
                    ...
                    if($gallery->status=="public")//only record for public galleries
                         ...
                         'content' => apply_filters( 'gallery_activity_created_gallery', sprintf( __( '%s created the gallery %s', 'bp-gallery'), bp_core_get_userlink( $bp->loggedin_user->id ), '<a href="' . bp_get_gallery_permalink( $gallery ) . '">' . esc_attr( $gallery->title ) . '</a>' ) ),
...
//do action gallery created
         ...
         //
                bp_core_redirect(bp_get_media_upload_link($gallery));//redirect to upload page for this gallery
                 ...
                 bp_core_add_message(__( 'Error in gallery creation.','bp-gallery' ), 'error' );
             }
  }
 }
...
function gallery_action_upload_media($owner_type="user",$owner_id='',$status='public'){
    ...
    $error=false;
    $message='';
//if we are here, it may be an upload request.
    if(isset($_REQUEST['gallery_upload_type'])){
   
        $upload_type=$_REQUEST['gallery_upload_type'];
        //check if activity upload
        if($upload_type=='activity_upload'){
            //upload for activity
            wp_verify_nonce('save_gallery_media');//check for the referrer
            $response=array();
            $gallery=bp_get_gallery(intval($_POST['galleries-list']));

            if(!$gallery->id){
                  $error=true;
                  $message=__('Please select a gallery to upload files','bp-gallery');
        ...
        //
                    ...
                    $message=__('Uploaded successfully','bp-gallery');
               
                }
                else{
                    $error=true;
                    $message=__('There was a problem in uploading. Please try again later!','bp-gallery');
                }
        ...
        bp_core_redirect(wp_get_referer());
    }//check if gallery cover upload
    else if($upload_type=='cover_upload'){
  ...
  if ( isset( $_POST['save-media'] ) ) {
       ...
       $response=array();
        ...
        if(gallery_server_unable_to_handle_upload()){
            $error=true;
            $message=__("Server can not handle this much amount of data. Please upload a smaller file or ask your server administrator to change the settings.","bp-gallery");
        ...
        if(!$error) {
            ...
            if(!$media_type==$gallery->gallery_type){
                $error = true;
                $message=sprintf( __('This file type is not allowed in current Gallery. You must upload a file which is %s', 'bp-gallery'), gallery_get_verbose_explanation_for_extension($gallery->gallery_type ) );
            }
            
            if(!$error&&!gallery_user_can_upload($gallery)) {
                $error=true;
                $message=__("You don't have sufficient permissions.", "bp-gallery");
        ...
        if(empty($upload_info['error'])) {
            ...
            if(!($id=gallery_add_media(array("title"=>stripslashes($title),
                    ...
                    "enable_wire"=>1)))){//if there is an error
                        ...
                        $error=true;
                        $message=__('There were problems saving your information.', 'bp-gallery');
            ...
            $error=true;
            $message=sprintf(__('Garr... %s', 'bp-gallery' ), $upload_info['error'] );
