<?php
/** Plugin Name: BP Gallery(BuddyDev)
  * Version: 1.3.9
  * Author: BuddyDev,Brajesh Singh
  * Author URI: http://buddydev.com/members/sbrajesh/
  * Plugin URI: http://buddydev.com/plugins/bp-gallery/
  * Description: The Ultimate Gallery Plugin for BuddyPress. Must be used with current  BP 1.6+
  * Date: April 18, 2014
  * # with support for mediaelement player
 */
    /* Define Constants */
    define ( 'BP_GALLERY_IS_INSTALLED', 1 );
    define ( 'BP_GALLERY_VERSION', '1.3.7' );
    define ( 'BP_GALLERY_DB_VERSION', 32);
    ...
    add_action( 'bp_include', 'bp_gallery_init', 1 );//Hook to bp_loaded for loading the plugin, making it bp aware :)
...
function bp_gallery_init(){
    ...
    require ( BP_GALLERY_PLUGIN_DIR . 'core/theme-compat.php' );
...
add_action( 'wp_loaded', 'bp_gallery_load_theme_functions' );

function bp_gallery_load_theme_functions(){

    if( ! function_exists( 'bp_is_active' ) )
        return ;//don't give fatal errors when BP is disabled
    
    $gallery_dir = bp_gallery_get_template_dir();
    
        //load the pluggable functions.php from theme/gallery/functions.php
    if( $gallery_dir && file_exists( $gallery_dir . '/functions.php' ) )
        include_once( $gallery_dir . '/functions.php' );//allow for the separation of the theme based players etc
...
function bp_gallery_setup_nav() {
    ...
    $bp->gallery->is_single_media = false;
    $bp->gallery->is_single_gallery = false;
  if( $gallery_id = BP_Gallery_Gallery::gallery_exists( bp_action_variable( 0 ), $bp->gallery->owner_type, $bp->gallery->owner_id ) ) {
    //in case of user gallery , group gallery, event gallery it will work for single gallery
        ...
        if( $media_id = BP_Gallery_Media::media_exists( bp_action_variable( 1 ), $gallery_id ) ) {
...
//			$bp->is_item_mod = gallery_is_user_mod( $bp->loggedin_user->id, $bp->gallery->current_gallery->id);
		...
		/* Should this gallery be visible to the logged in user? */
                    ...
                    $bp->gallery->current_gallery->is_gallery_visible_to_member = (function_exists('bp_get_group_status')&& bp_is_group_single() && 'public' == bp_get_group_status(groups_get_current_group())) ||$bp->gallery->current_gallery->is_user_member  ? true : false;
...
if( bp_is_current_component( $bp->gallery->slug ) ) {
 ...
 } elseif( $bp->current_action == $bp->gallery->slug ) {
...
function bp_gallery_add_admin_menu() {
	...
	/* Add the administration tab under the "BuddyPress" tab for site administrators */
        if( is_multisite() )
            add_submenu_page( 'settings.php', __("Gallery", 'bp-gallery'), __("BP Gallery", 'bp-gallery'), 'manage_options', "gallery_admin_settings", "gallery_admin_settings" );
        else
           add_options_page(  __("Gallery", 'bp-gallery'), __("BP Gallery", 'bp-gallery'), 'manage_options', "gallery_admin_settings", "gallery_admin_settings" );
}
...
//hide from wp repo updated because someone did register another plugin with same name
function bp_gallery_hide_from_wporg_update_check( $r, $url ) {
...
add_filter( 'http_request_args', 'bp_gallery_hide_from_wporg_update_check', 5, 2 );
