...
function bp_gallery_show_gallery_create_form() {
  ...
  bp_gallery_locate_template( array( 'gallery/single/create-form.php' ), true );
  exit(0);
}
...
function show_media_edit_form() {
    ...
    if(!empty($_POST["gallery_id"])&&gallery_is_user_admin($bp->loggedin_user->id,intval($_POST["gallery_id"]))) {
        ...
        bp_gallery_locate_template( array( 'gallery/single/edit.php' ), true );
    }
    exit(0);
}
...
function bp_gallery_show_activity_upload_form(){
    ...
    $type=$_POST["gallery_type"];
     ...
     if(gallery_get_gallery_count_by_gallery_type($type, $owner_type, $owner_id))
        bp_gallery_locate_template( array( 'gallery/single/media/activity-upload-form.php' ), true );
...
//change gallery cover
    ...
    if(gallery_is_user_admin($bp->loggedin_user->id,$gallery->id)) {
        $gallery=bp_gallery_save_cover_image();//bad idea
        $response["data"]=build_json_for_gallery($gallery);
...
function bp_gallery_save_cover_image(){
    //change gallery cover
    //gallery_fix_flash_upload();//fix the problems with flash upload
    global $bp,$current_user;
    //check rfrerer
...
add_action('admin_init','gallery_fix_flash_upload');
...
//let us build our response for javascript
        $media_type=bp_gallery_get_media_type_from_file($file['file']);
        if(!$media_type==$gallery->gallery_type)
            $response["error"]=array("msg"=> sprintf( __('This file type is not allowed in current Gallery. You must upload a file which is %s', 'bp-gallery'), gallery_get_verbose_explanation_for_extension($gallery->gallery_type ) ) );
        if(!gallery_user_can_upload($gallery)) {
...
//upload file
        ...
        if(empty($upload_info['error'])) {
            ...
            $description= isset($_POST["media_description"] ) ? $_POST["media_description"]: '';
            $image_meta=null;
            if($media_type=="photo"){
       ...
       do_action( 'gallery_media_upload_complete', $media );
        ...
        $response["error"]= array( 'msg' => sprintf( __( 'Garr... %s', 'bp-gallery' ), $upload_info['error'] ) );
    }
...
function gallery_upload_from_activity(){
  ...
  if(empty($response['error'])){
        ...
        $activity_action=sprintf( __( "%s uploaded a %s to %s" , 'bp-gallery'),bp_core_get_userlink( $bp->loggedin_user->id ),$type,'<a href="' . bp_get_gallery_permalink( $gallery ) . '">' . esc_attr( $gallery->title ) . '</a>'.":"  );
...
//
		...
		<?php while ( bp_activities() ) : bp_the_activity(); ?>
			<?php bp_gallery_locate_template( array( 'activity/entry.php' ), true ) ?>
		<?php endwhile; ?>
	 <?php endif;
...
add_action("wp_ajax_nopriv_gallery_filter","bp_gallery_object_template_loader");
function bp_gallery_object_template_loader(){
  ...
  //  add_filter("bp_dtheme_ajax_querystring","bp_gallery_filter_directory_data",12,7);
    bp_gallery_locate_template( array( "gallery/gallery-loop.php" ), true );
    exit(0);
}
...
function gallery_fix_flash_upload() {
    ...
    $ajax_action=isset($_REQUEST['action'])?$_REQUEST['action']:false;
   
    //we only need resetting user in these cases
    if(!in_array($ajax_action,array('upload_from_activity','save_gallery_media_bulk','save_gallery_cover')))
        return;
  
    //we only need if doing ajax and the action is save_media_bulk?
...
if($is_media_comment){//for media comment
      ...
      $activity_content=bp_get_gallery_media_thumb_html($media,true);//sprintf( __( '%s uploaded the %s to %s  %s', 'bp-gallery'), bp_core_get_userlink( $bp->loggedin_user->id ), $media_type_name, '<a href="' . bp_get_gallery_permalink( $gallery ) . '">' . esc_attr( $gallery->title ) . '</a>',);
    ...
    $secondary_id=$media->id;
        ...
        $user_id=$bp->loggedin_user->id;
         ...
         $activity_content=$content;//bp_get_gallery_media_thumb_html($media,true);//sprintf( __( '%s uploaded the %s to %s  %s', 'bp-gallery'), bp_core_get_userlink( $bp->loggedin_user->id ), $media_type_name, '<a href="' . bp_get_gallery_permalink( $gallery ) . '">' . esc_attr( $gallery->title ) . '</a>',);
...
if ( bp_has_activities ( "action=$action&show_hidden=1&secondary_id=".$secondary_id)) : ?>
		<?php while ( bp_activities() ) : bp_the_activity(); ?>
			<?php if($is_media_comment)
                            bp_gallery_locate_template( array( 'gallery/single/media/activity-entry.php' ), true );
                        else             
                         bp_gallery_locate_template( array( 'activity/entry.php' ), true );
                        ?>
		<?php endwhile; ?>
	 <?php endif;
...
function bp_gallery_print_comments_for_media_ajax(){
   ...
   // echo $media_id;
    include(bp_gallery_locate_template( array( 'gallery/single/media/activity.php' ), false ) );
    exit(0);
}
add_action("wp_ajax_gallery_load_media_comments","bp_gallery_print_comments_for_media_ajax");
add_action("wp_ajax_nopriv_gallery_load_media_comments","bp_gallery_print_comments_for_media_ajax");
