...
function gallery_screen_edit_gallery() {
    global $bp;
    if( ( bp_is_current_action('manage') && bp_is_current_component( $bp->gallery->slug ) ) || ( bp_is_current_action( $bp->gallery->slug ) && bp_is_action_variable('manage',0))){
    ...
    switch( $action ){
            ...
            case "cover-upload":
                 ...
                 $g = bp_gallery_save_cover_image();
                 bp_core_add_message( __( 'Cover updated','bp-gallery' ) );
...
//echo $type;
        ...
        //check_admin_referer('gallery_edit_save',"_wpnonce-edit-save-gallery");
            ...
            foreach( $_POST["gallery_title"] as $gallery_id => $gallery_title ) {
                    ...
                    gallery_update_gallery_details(
                                        array(
                                            "id"            => $gallery_id,
                                            "title"         => $gallery_title,
...
add_action( "bp_screens", "gallery_screen_edit_gallery", 1 );
...
function gallery_screen_single_gallery_view(){
    ...
    elseif( $bp->gallery->is_single_gallery )
        $type = $bp->gallery->current_gallery->gallery_type;
    ...
    if( !bp_is_gallery_enabled_for( $component ) || !gallery_is_enabled_for_current_item() )
     ...
     if( $bp->gallery->is_single_media )
        bp_core_load_template( apply_filters( 'gallery_single_media_template', 'gallery/index' ) );//'gallery_single_media_'.$type
    else if( $bp->gallery->is_single_gallery )
        bp_core_load_template( apply_filters( 'gallery_single_gallery_template', 'gallery/index' ) );//'gallery_single_gallery_'.$type

}
add_action( 'bp_screens', 'gallery_screen_single_gallery_view', 6 );
...
function gallery_upload_buttons_for_activity(){
    ...
    if( bp_is_user() && !bp_is_gallery_enabled_for( 'user' ) )
        return;
    
    if( function_exists( 'bp_is_group' ) && bp_is_group() && !bp_is_gallery_enabled_for( 'groups' ) )
        return;
