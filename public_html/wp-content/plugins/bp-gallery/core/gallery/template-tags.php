...
function bp_has_galleries( $args = '' ) {
    ...
    if ( isset($bp->gallery->is_home)&&$bp->gallery->is_home ) {
     ...
     else if (isset($bp->gallery->current_gallery)&& $bp->gallery->current_gallery->slug ) {//if single gallery/media
...
function bp_gallery_date_created() {
	...
	function bp_get_gallery_date_created( $gallery = false ) {
		...
		return  apply_filters( 'bp_get_gallery_date_created', date_i18n( get_option( 'date_format' ), $gallery->date_created ) );
	}
...
function bp_gallery_last_updated() {
	...
	function bp_get_gallery_last_updated( $gallery = false ) {
		...
		return sprintf( __( 'Updated: %s', 'bp-gallery' ), apply_filters( 'bp_get_gallery_date_created', date_i18n( get_option( 'date_format' ), $gallery->date_updated ) ) );
	}
...
function bp_get_gallery_home_url(){
    ...
    $user_gallery_link=bp_core_get_user_domain($user_id).BP_GALLERY_SLUG;
...
if(!empty($type_name))
    $message=sprintf(__("There are no %s yet.","bp-gallery"),  strtolower ($type_name));
else
...
function bp_user_gallery_admin_tabs( ) {//do we really need it
    ...
    <?php if(gallery_is_valid_gallery_type("photo")):?>
        <li<?php if ( 'photo' == $current_tab ) : ?> class="current"<?php endif; ?> data-gallery-type="photo"><a  href="<?php echo bp_get_gallery_home_url();?>/my-galleries/photo"> <?php _e("Photo","bp-gallery");?></a></li>
    <?php endif;?>
    <?php if(gallery_is_valid_gallery_type("video")):?>
        <li<?php if ( 'video' == $current_tab ) : ?> class="current"<?php endif; ?> data-gallery-type="video"><a href="<?php echo bp_get_gallery_home_url();?>/my-galleries/video"> <?php _e("Video","bp-gallery");?></a></li>
    <?php endif;?>
    <?php if(gallery_is_valid_gallery_type("audio")):?>
        <li<?php if ( 'audio' == $current_tab ) : ?> class="current"<?php endif; ?> data-gallery-type="audio"><a href="<?php echo bp_get_gallery_home_url();?>/my-galleries/audio"> <?php _e("Audio","bp-gallery");?></a></li>
<?php endif;?>

 <?php
    do_action('user_gallery_admin_tabs');
}
function bp_user_gallery_admin_tabs_theme_compat( ) {//do we really need it
    global $bp;
    $current_tab = isset($bp->action_variables[0])?$bp->action_variables[0]:false;
    ?>
...
function bp_directory_gallery_search_form() {
	...
	<form action="" method="get" id="search-gallery-form">
		<label><input type="text" name="s" id="gallery_search" value="<?php echo esc_attr( $search_value ) ?>"  onfocus="if (this.value == '<?php _e( 'Search anything...', 'bp-gallery' ) ?>') {this.value = '';}" onblur="if (this.value == '') {this.value = '<?php _e( 'Search anything...', 'bp-gallery' ) ?>';}" /></label>
